...
(function ($) {
  ...
  $(document).ready(function () {
    const content = [
      ...
      },
      {
        ...
        keywords: [
          ...
          { keyword: "Paver Maintenance", url: "paver-maintenance.html" },
    ...
    $(".df-search-category a").on("click", function (e) {
      ...
      const categoryResults = content.find(
    ...
    function displayCategoryResults(results) {
      ...
      results.forEach((result) => {
        resultsContainer.append(
          `<p><a href="${result.url}">${result.keyword}</a></p>`
        );
      });
    }
    ...
    function searchContent(query) {
      return content.filter(
        (item) =>
          ...
          item.keywords.some((keywordObj) =>
            keywordObj.keyword.toLowerCase().includes(query)
          )
      );
    }
